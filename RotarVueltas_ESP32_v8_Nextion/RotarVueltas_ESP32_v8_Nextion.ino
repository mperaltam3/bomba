/*
 * RotarVueltas_ESP32_v8_Nextion.ino
 * Version de calibracion de RotarVueltas_ESP32_v7_Nextion.
 *
 * Diferencia con v7:
...
#include "D:/DOCUMENTOS MICHAEL PERALTA/BOMBA/motor2/FTServo_Arduino/examples/SMS_STS/RotarVueltas_ESP32_v8_Nextion/SCServo.h"
...
void rotarVueltas(float n, float minutos) {
  ...
  while (millis() - tStart < tTotal) {
    ...
    revisarOcultarAlertaBurbuja();
    if (monitorearCorriente()) {
      enviarSerial("[SOBRECORRIENTE] Movimiento pausado. Envia '" + String(COMANDO_REGRESO) +
      ...
      if (decision == 2) {
        enviarSerial("Rotacion cancelada por interrupcion durante la pausa de sobrecorriente. "
                     "Vueltas acumuladas: " + String(vueltasActuales(), 3));
        return;
      }
      if (decision == 1) {
        enviarSerial("Rotacion cancelada por sobrecorriente: yendo a home. Vueltas acumuladas: " +
                     String(vueltasActuales(), 3));
        secuenciaRegreso();
        return;
      }
      ...
      sms_sts.WriteSpe(SERVO_ID, speedVal, 0);
      sonarInicioMovimiento();
      enviarSerial("[SOBRECORRIENTE] Reanudando movimiento...");
    }
    actualizarConteoVueltas();
