/*
 * RotarVueltas_ESP32_v17_Nextion.ino
 * Version de calibracion de RotarVueltas_ESP32_v16_Nextion.
 *
 * Diferencia con v16:
...
#include "D:/DOCUMENTOS MICHAEL PERALTA/BOMBA/motor2/FTServo_Arduino/examples/SMS_STS/RotarVueltas_ESP32_v17_Nextion/SCServo.h"
...
#define MARGEN_CORRIENTE_PORCENTAJE 40.0f // 5 funciona pero no tan bien
...
// empujon real). ---
#define MARGEN_CORRIENTE_MINIMO_MA 10.0f
...
//     Concentradas en el extremo BAJO del rango (2%-25% de SPEED_MAX cubre
//     6 de los 10 puntos) porque en el uso real casi siempre se opera a
//     velocidades bajas/medias - las velocidades altas (mayor a 35%) se usan
//     poco, asi que les alcanza con menos resolucion (35%, 50%, 70%, 100%).
//     Esto da un umbral bien ajustado justo donde mas importa (velocidades
//     bajas, donde ademas MARGEN_CORRIENTE_PORCENTAJE al 12% deja menos
//     margen absoluto en mA). corrienteNormalMA[] queda en 0 y
//     calibracionCorrienteValida en false hasta que se corra la
...
const int16_t VELOCIDADES_CALIBRACION_CORRIENTE[NUM_PUNTOS_CALIBRACION_CORRIENTE] = {
  (int16_t)(SPEED_MAX * 0.02f),
  (int16_t)(SPEED_MAX * 0.05f),
  (int16_t)(SPEED_MAX * 0.08f),
  (int16_t)(SPEED_MAX * 0.12f),
  (int16_t)(SPEED_MAX * 0.17f),
  (int16_t)(SPEED_MAX * 0.25f),
  (int16_t)(SPEED_MAX * 0.35f),
  (int16_t)(SPEED_MAX * 0.50f),
  (int16_t)(SPEED_MAX * 0.70f),
  SPEED_MAX
};
...
// dos margenes: el porcentual (MARGEN_CORRIENTE_PORCENTAJE) y el aditivo fijo
// (MARGEN_CORRIENTE_MINIMO_MA). Esto evita que a velocidades bajas -donde la
...
float corrienteUmbralParaVelocidad(int16_t velocidad) {
  ...
  float umbralPorcentual = corrienteNormalInterpolada * (1.0f + MARGEN_CORRIENTE_PORCENTAJE / 100.0f);
...
void calibrarCorrientePorVelocidad() {
  ...
  enviarDebug("[CALIBRACION-CORRIENTE] Completa y guardada en memoria (no hace falta repetirla en el "
               "proximo arranque). Margen aplicado: el mayor entre +" + String(MARGEN_CORRIENTE_PORCENTAJE, 0) +
