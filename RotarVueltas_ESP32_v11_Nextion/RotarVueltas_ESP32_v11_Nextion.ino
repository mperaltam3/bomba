/*
 * RotarVueltas_ESP32_v11_Nextion.ino
 * Version de calibracion de RotarVueltas_ESP32_v10_Nextion.
 *
 * Diferencia con v10:
 *   - Ataca la CAUSA RAIZ de por que el controlador de vueltas+tiempo (ver
 *     "Diferencia con v9" abajo) arrancaba demasiado rapido y tenia que
 *     pasarse el resto del giro frenando para compensar: RPM_POR_UNIDAD
 *     (el factor que convierte RPM deseada a velocidad de registro del
 *     servo) era una CONSTANTE fija (0.0146), medida en CalibracionRPM.ino
 *     con el servo SIN carga real. La friccion/carga del sistema montado
 *     (motor + bomba + manguera) hace que el servo alcance, para la misma
 *     velocidad comandada, una RPM real distinta a la de banco - por eso
 *     la velocidad inicial (feedforward) ya arrancaba desviada, y el
 *     controlador solo lo notaba y corregia despues de acumular bastante
 *     adelanto/atraso.
 *
 *     RPM_POR_UNIDAD paso de "const float" a "float": ahora se puede
 *     actualizar en tiempo de ejecucion con el valor REAL medido bajo carga.
 *     Nueva funcion calibrarRpmPorUnidad(), disparada con el nuevo comando
 *     COMANDO_CALIBRAR ('k'/'K') por Serial/Nextion (mismo UART0 que 'r' y
 *     "vueltas tiempo"):
 *       1) Gira el servo a SPEED_CALIBRACION durante DURACION_CALIBRACION_MS,
 *          con el sistema YA MONTADO (misma carga real que se usara despues
 *          en "vueltas tiempo") - a diferencia de CalibracionRPM.ino, que es
 *          un sketch aparte pensado para un banco de pruebas sin carga.
 *       2) Mide las vueltas realmente giradas (mismo metodo que
 *          actualizarConteoVueltas()/vueltasActuales()) y calcula la RPM real.
 *       3) Actualiza RPM_POR_UNIDAD = rpm_real / SPEED_CALIBRACION.
 *     Como cualquier otro movimiento, respeta emergencia/fines de carrera
 *     (procesarInterrupciones()) y sobrecorriente (monitorearCorriente()),
 *     y se cancela sin tocar RPM_POR_UNIDAD si alguna de las dos ocurre
 *     durante la prueba. Al terminar, el eje queda desplazado por la prueba
 *     (igual que tras cualquier giro): se recomienda enviar COMANDO_REGRESO
 *     ('r') antes de la primera prueba real de "vueltas tiempo".
 *     RPM_POR_UNIDAD conserva 0.0146 como valor por defecto (el medido sin
 *     carga) por si no se calibra antes de girar, para no dejar la variable
...
#include "D:/DOCUMENTOS MICHAEL PERALTA/BOMBA/motor2/FTServo_Arduino/examples/SMS_STS/RotarVueltas_ESP32_v11_Nextion/SCServo.h"
...
// Factor que convierte RPM deseada a velocidad de registro del servo. Valor
// por defecto = el medido SIN carga en CalibracionRPM.ino; se sobreescribe
// con la medicion REAL bajo carga al ejecutar calibrarRpmPorUnidad() (ver
// "Diferencia con v10" arriba y COMANDO_CALIBRAR mas abajo). Por eso ya NO
// es "const": rotarVueltas() siempre debe usar el valor mas reciente.
float RPM_POR_UNIDAD = 0.0146f;
...
// --- Comando para disparar calibrarRpmPorUnidad() por Serial/Nextion ---
#define COMANDO_CALIBRAR 'k'   // se acepta 'k' o 'K'

// --- Calibracion de RPM_POR_UNIDAD bajo carga real: velocidad de registro
//     usada durante la prueba. Elegir una velocidad tipica de uso (ni muy
//     baja ni SPEED_MAX), para que el factor medido sea representativo de
//     las pruebas reales de "vueltas tiempo". ---
#define SPEED_CALIBRACION 1000

// --- Calibracion de RPM_POR_UNIDAD: duracion de la prueba. Mas duracion =
//     medicion mas precisa (promedia mejor el ruido de lectura de
//     posicion), a costa de tardar mas en tener el factor listo. ---
#define DURACION_CALIBRACION_MS 8000
...
// -----------------------------------------------------------------------
// calibrarRpmPorUnidad: mide la RPM REAL que alcanza el servo a
// SPEED_CALIBRACION unidades de velocidad, BAJO LA CARGA REAL del sistema ya
// montado (motor + bomba + manguera), y actualiza RPM_POR_UNIDAD con ese
// valor medido. Ataca la causa raiz de que rotarVueltas() no completara las
// vueltas solicitadas en tiempos largos: el valor original (0.0146) se midio
// con el servo SIN carga (CalibracionRPM.ino), y la friccion/carga real hace
// que la conversion real rpm-por-unidad sea distinta a la de banco.
// Se dispara con COMANDO_CALIBRAR ('k'/'K') por Serial/Nextion. Igual que
// rotarVueltas(), respeta emergencia/fines de carrera (procesarInterrupciones())
// y sobrecorriente (monitorearCorriente()) durante la prueba; si cualquiera
// de las dos ocurre, la cancela sin tocar RPM_POR_UNIDAD.
// Al terminar, el eje queda desplazado por la prueba (igual que tras
// cualquier giro): se recomienda enviar COMANDO_REGRESO ('r') antes de la
// primera prueba real de "vueltas tiempo".
// -----------------------------------------------------------------------
void calibrarRpmPorUnidad() {
  enviarDebug("[CALIBRACION-RPM] Midiendo RPM real a velocidad " + String(SPEED_CALIBRACION) +
               " durante " + String(DURACION_CALIBRACION_MS / 1000) + " s (con la carga real conectada)...");
  sonarInicioMovimiento();

  iniciarConteoVueltas();
  velocidadActual = SPEED_CALIBRACION;
  sms_sts.WriteSpe(SERVO_ID, SPEED_CALIBRACION, 0);

  unsigned long tInicio    = millis();
  bool          cancelado  = false;

  while (millis() - tInicio < DURACION_CALIBRACION_MS) {
    if (procesarInterrupciones()) {
      cancelado = true;
      break;
    }
  ...
  if (cancelado) {
    enviarDebug("[CALIBRACION-RPM] Cancelada por interrupcion o sobrecorriente. RPM_POR_UNIDAD sin cambios (" +
                 String(RPM_POR_UNIDAD, 4) + ").");
    return;
  }

  float vueltas     = vueltasActuales();
  float duracionMin = (tFin - tInicio) / 60000.0f;
  float rpmReal     = vueltas / duracionMin;
  float nuevoFactor = rpmReal / (float)SPEED_CALIBRACION;

  if (vueltas <= 0.0f || nuevoFactor <= 0.0f) {
    enviarDebug("[CALIBRACION-RPM] Medicion invalida (el servo no giro). RPM_POR_UNIDAD sin cambios (" +
                 String(RPM_POR_UNIDAD, 4) + ").");
    return;
  }

  RPM_POR_UNIDAD = nuevoFactor;

  enviarDebug("[CALIBRACION-RPM] Vueltas medidas: " + String(vueltas, 3) +
               " | RPM real: " + String(rpmReal, 3) +
               " | Nuevo RPM_POR_UNIDAD: " + String(RPM_POR_UNIDAD, 4));
  enviarDebug("[CALIBRACION-RPM] Completa. Envia '" + String(COMANDO_REGRESO) +
               "' para regresar a home antes de iniciar una prueba de vueltas.");
  sonarFinMovimiento();
}
...
void setup() {
  ...
  enviarDebug("Servo listo. Interrupciones activas. Modo calibracion.\r\n"
               ...
               "O envia '" + String(COMANDO_CALIBRAR) + "' para medir RPM_POR_UNIDAD "
               "bajo la carga real (recomendado antes de la primera prueba).\r\n"
...
void loop() {
  ...
  if (Serial.available()) {
    ...
    // Comando de una letra ('k'/'K'): dispara la calibracion de
    // RPM_POR_UNIDAD bajo la carga real ya conectada (ver COMANDO_CALIBRAR).
    if (primerByte == 'k' || primerByte == 'K') {
      Serial.read();
      while (Serial.available()) Serial.read();  // limpiar resto de la linea
      calibrarRpmPorUnidad();
      return;
    }
